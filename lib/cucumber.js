...
Cucumber.Ast.TreeWalker = function(features, supportCodeLibrary, listeners) {
  ...
  var self = {
    ...
    // broadcastUserFunction(message[, param1[, param2[, ...]]], userFunction, callback)
    // payload is the array containing messages and all paramX.
    ...
    extractMessagePayloadFromArguments: function extractMessagePayloadFromArguments(argumentsObject) {
      var argumentsArray = Cucumber.Util.Arguments(argumentsObject);
      var payload        = argumentsArray.slice(
    ...
    wrapUserFunctionAndAfterMessageBroadcast: function wrapUserFunctionAndAfterMessageBroadcast(userFunction, payload, callback) {
      var callAfterMessageBroadcast = self.wrapAfterMessageBroadcast(payload, callback);
      return function callUserFunctionAndBroadcastAfterMessage() {
        userFunction(callAfterMessageBroadcast);
      };
    },

    wrapAfterMessageBroadcast: function wrapAfterMessageBroadcast(payload, callback) {
      return function() { self.broadcastAfterMessage(payload, callback); };
    },
...
Cucumber.Ast.TreeWalker.NON_PAYLOAD_LEADING_PARAMETERS_COUNT  = 0;
Cucumber.Ast.TreeWalker.NON_PAYLOAD_TRAILING_PARAMETERS_COUNT = 2;
